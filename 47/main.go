...
func createTestCases() []struct {
	...
	return []struct {
		...
		{[]int{1, 1, 1}, "测试1: [1,1,1]"},
		{[]int{1, 2, 1}, "测试2: [1,2,1]"},
		{[]int{2, 2, 1, 1}, "测试3: [2,2,1,1]"},
		{[]int{1, 1, 2, 2}, "测试4: [1,1,2,2]"},
		{[]int{1, 2, 3, 1}, "测试5: [1,2,3,1]"},
		{[]int{0, 1, 0, 0, 9}, "测试6: [0,1,0,0,9]"},
		{[]int{1, 1, 1, 2}, "测试7: [1,1,1,2]"},
		{[]int{1, 2, 2, 3}, "测试8: [1,2,2,3]"},
	}
}
...
func validateResult(nums []int, result [][]int) bool {
	// 验证结果数量
	expectedCount := countUniquePermutations(nums)
...
// 计算不重复排列的数量
func countUniquePermutations(nums []int) int {
	// 统计每个数字的出现次数
...
func main() {
	...
	algorithms := []struct {
		...
		{"递归回溯算法", permuteUnique1},
		{"交换回溯算法", permuteUnique2},
		{"迭代回溯算法", permuteUnique3},
		{"字典序算法", permuteUnique4},
	}
	...
	// 算法分析
	fmt.Println("=== 算法分析 ===")
	fmt.Println("全排列II问题的特点:")
	fmt.Println("1. 需要生成包含重复数字的序列的所有不重复全排列")
	fmt.Println("2. 每个元素只能使用一次")
	fmt.Println("3. 需要处理重复元素，避免生成重复的排列")
	...
	fmt.Println("4. 去重处理：处理重复元素，避免生成重复的排列")
	fmt.Println("5. 剪枝优化：避免重复选择同一元素和重复的排列")
